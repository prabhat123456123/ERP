...
module.exports = {
	...
	convertUniTest: function (message, lang = "hindi_2304") {
		...
		if (lg == "bangla") arrayT = new Array("98", "99", "9A", "9B", "9C", "9D", "9E", "9F");
		else if (lg == "gurmukhi")
			arrayT = new Array("A0", "A1", "A2", "A3", "A4", "A5", "A6", "A7");
		else if (lg == "gujarati")
			arrayT = new Array("A8", "A9", "AA", "AB", "AC", "AD", "AE", "AF");
		else if (lg == "oriya") arrayT = new Array("B0", "B1", "B2", "B3", "B4", "B5", "B6", "B7");
		else if (lg == "tamil") arrayT = new Array("B8", "B9", "BA", "BB", "BC", "BD", "BE", "BF");
		else if (lg == "telugu") arrayT = new Array("C0", "C1", "C2", "C3", "C4", "C5", "C6", "C7");
		else if (lg == "kannada")
		...
		while (i < line.length) {
			...
			while (true) {
				...
				if (akshar.replace(/^\s+|\s+$/g, "").length > 0) aksU = u2r_getRupA(akshar);
				if (matra.replace(/^\s+|\s+$/g, "").length > 0) matU = u2r_getRupM(matra);
				else {
					if (aksU.length > 0 && vowel.indexOf(aksU.charAt(aksU.length - 1)) < 0)
						matU = "a";
					if (aksU.length > 0 && number.indexOf(aksU.charAt(aksU.length - 1)) >= 0)
						matU = "";
				}
